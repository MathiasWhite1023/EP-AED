...
### 2.5 Novos Cenários de Teste (Diversidade Semântica)
Além dos textos padronizados baseados no enunciado, foram criados três novos arquivos com temáticas e vocabulários distintos para validar a robustez do indexador em diferentes contextos linguísticos:

- **teste_novo_curto.txt**: Texto sobre exploração espacial (inglês), focado em termos específicos.
- **teste_novo_medio.txt**: Texto sobre a história da computação, com maior densidade de termos técnicos.
- **teste_novo_variado.txt**: Texto combinando parágrafos sobre natureza, música, culinária, esportes, arquitetura e literatura, visando maximizar a variabilidade do vocabulário.
- **teste_gigante.txt**: Um arquivo de estresse de aproximadamente 127MB, gerado pela replicação massiva do texto variado, para testar limites de alocação de memória.
...
- Tabela 1 - Construção do índice (Arquivos Originais)
| Arquivo | Linhas | Palavras Únicas | Estrutura | Comparações (Construção) |
| ------- | ------ | --------------- | --------- | ------------------------ |
- Tabela 1 - Construção do índice (Arquivos Originais)
| Arquivo | Linhas | Palavras Únicas | Estrutura | Comparações (Construção) |
| ------- | ------ | --------------- | --------- | ------------------------ |
| texto_curto.txt | 11     | 70              | Lista     | 3412                     |
| texto_curto.txt | 11     | 70              | Árvore    | 691                      |
| texto_medio.txt | 129    | 74              | Lista     | 38346                    |
| texto_medio.txt | 129    | 74              | Árvore    | 7966                     |
| texto_longo.txt | 1219   | 87              | Lista     | 388193                   |
| texto_longo.txt | 1219   | 87              | Árvore    | 80810                    |
...
### 3.6. Resultados dos Novos Arquivos (Variados)
Para validar a robustez do sistema, os testes foram replicados nos novos arquivos criados, que possuem maior riqueza de vocabulário.

- Tabela 5 - Construção com Novos Textos
| Arquivo | Tema | Estrutura | Comparações (Construção) |
| ------- | ---- | --------- | ------------------------ |
- Tabela 5 - Construção com Novos Textos
| Arquivo | Tema | Estrutura | Comparações (Construção) |
| ------- | ---- | --------- | ------------------------ |
| teste_novo_curto.txt   | Espaço       | Lista     | 2522   |
| teste_novo_curto.txt   | Espaço       | Árvore    | 469    |
| teste_novo_medio.txt   | Computação   | Lista     | 19853  |
| teste_novo_medio.txt   | Computação   | Árvore    | 1831   |
| teste_novo_variado.txt | Diversos     | Lista     | 40607  |
| teste_novo_variado.txt | Diversos     | Árvore    | 2894   |

### 3.7. Discussão Detalhada dos Resultados
A análise comparativa entre os arquivos originais e os novos cenários de teste nos permite compreender profundamente como o conteúdo do texto influencia o desempenho das estruturas de dados.

#### Impacto do Vocabulário na Lista
Ao contrastarmos o arquivo **texto_medio.txt** (repetitivo) com o **teste_novo_variado.txt** (diverso), observamos que a Lista Encadeada é extremamente sensível à riqueza do vocabulário. Embora o arquivo variado seja menor em número total de linhas, ele possui 266 palavras únicas, contra apenas 74 do arquivo médio repetitivo.

Essa diferença triplicou o esforço necessário para a busca linear na Lista, elevando o custo de construção para mais de 40.000 comparações. Isso confirma que, em uma lista encadeada não ordenada, o custo médio de inserção cresce linearmente com o tamanho do vocabulário indexado ($V$).

#### Estabilidade e Eficiência da Árvore
Em contrapartida, a Árvore Binária demonstrou uma estabilidade notável. Mesmo com o aumento da complexidade do vocabulário no texto variado, o número de comparações permaneceu baixo (2.894). Isso ocorre porque a árvore reduz o espaço de busca de forma logarítmica ($O(\log V)$).

#### Quadro Resumo
A tabela a seguir consolida essa divergência de desempenho, evidenciando como a vantagem da árvore se amplia em textos com maior diversidade semântica:

- Tabela 6 - Comparativo Geral
| Arquivo | Tipo | Palavras Únicas (V) | Custo Lista | Custo Árvore | Ratio (L/A) |
| :--- | :--- | :---: | :---: | :---: | :---: |
| teste_novo_curto.txt | Variado | 67 | 2522 | 469 | 5.3x |
| texto_curto.txt | Repetitivo | 70 | 3412 | 691 | 4.9x |
| texto_medio.txt | Repetitivo | 74 | 38346 | 7966 | 4.8x |
| teste_novo_medio.txt | Variado | 185 | 19853 | 1831 | 10.8x |
| teste_novo_variado.txt | Variado | 266 | 40607 | 2894 | 14.0x |
| texto_longo.txt | Repetitivo | 87 | 388193 | 80810 | 4.8x |

Conclui-se que a eficiência da Árvore Binária não é apenas superior em números absolutos, mas torna-se exponencialmente mais vantajosa à medida que o texto se torna mais complexo e realista.
...
### 4.5 Teste de Stress (Alocação Dinâmica)
Como etapa final de validação, o sistema foi submetido a um teste de carga extrema com o arquivo `teste_gigante.txt` (127 MB). O programa foi capaz de alocar e processar o arquivo com sucesso na estrutura de Árvore, demonstrando que a implementação de alocação dinâmica de memória é robusta e capaz de escalar para volumes de dados muito superiores aos exigidos inicialmente, limitada apenas pela memória física disponível no sistema.
